...
t=1;
v_fixed = zeros(5, 4);
v_fixed(1,1:4) = [fixed_vertices(1, 1:2),fixed_vertices(2, 1:2)] ;
v_fixed(2,1:4) = [fixed_vertices(2, 1:2),fixed_vertices(3, 1:2)];
...
for i=1:length(FV1) %calculate A for each triangle. 
    ...
    q_t = 1/sin(angles{2,i})*(sin((1-t)*angles{2,i}))*q0 + sin(t*angles{2,i})/sin(angles{2,i})*q{2,i}; %slerp
    Rot_t= [1-2*q_t(3)^2 - 2*q_t(4)^2, 2*q_t(2)*q_t(3)+2*q_t(1)*q_t(4); 2*q_t(2)*q_t(3)-2*q_t(1)*q_t(4), 1-2*q_t(2)^2 - 2*q_t(4)^2];
    At = Rot_t*((1-t)*eye(2) +  t*S{2,i});
...
b(4*length(FV1)+1)= (1-t)*fixed_vertices(2,1)+t*fixed_vertices(2+1,1);  %constraints for fixed vertex.
b(4*length(FV1)+2)= (1-t)*fixed_vertices(2,2)+t*fixed_vertices(2+1,2);
...
v_fixed(3,1:4) = [V_3(1,1), V_3(1,2), fixed_vertices(4,1:2)];
    
for i=1:length(FV1) %calculate A for each triangle. 
    P3 = [V_3(FV1(i,1),1), V_3(FV1(i,1),2); V_3(FV1(i,2),1), V_3(FV1(i,2),2); V_3(FV1(i,3),1), V_3(FV1(i,3),2)];
    ...
    q_t = 1/sin(angles{3,i})*(sin((1-t)*angles{3,i}))*q0 + sin(t*angles{3,i})/sin(angles{3,i})*q{3,i}; %slerp
    Rot_t= [1-2*q_t(3)^2 - 2*q_t(4)^2, 2*q_t(2)*q_t(3)+2*q_t(1)*q_t(4); 2*q_t(2)*q_t(3)-2*q_t(1)*q_t(4), 1-2*q_t(2)^2 - 2*q_t(4)^2];
    At = Rot_t*((1-t)*eye(2) +  t*S{3,i});
    b(4*(i-1)+1:4*(i-1)+4)=[At(1,1), At(1,2), At(2,1), At(2,2)]'; %build up matrix b, for min ||Ax-b||.
...
b(4*length(FV1)+1)= (1-t)*fixed_vertices(3,1)+t*fixed_vertices(3+1,1);  %constraints for fixed vertex.
b(4*length(FV1)+2)= (1-t)*fixed_vertices(3,2)+t*fixed_vertices(3+1,2);
A(4*length(FV1)+1,1) = 1;
A(4*length(FV1)+2,2)=1;
...
v_fixed(4,1:4) = [V_4(1,1), V_4(1,2), fixed_vertices(5,1:2)];

for i=1:length(FV1) %calculate A for each triangle.
    
    P4 = [V_4(FV1(i,1),1), V_4(FV1(i,1),2); V_4(FV1(i,2),1), V_4(FV1(i,2),2); V_4(FV1(i,3),1), V_4(FV1(i,3),2)];
    ...
    q_t = 1/sin(angles{4,i})*(sin((1-t)*angles{4,i}))*q0 + sin(t*angles{4,i})/sin(angles{4,i})*q{4,i}; %slerp
    Rot_t= [1-2*q_t(3)^2 - 2*q_t(4)^2, 2*q_t(2)*q_t(3)+2*q_t(1)*q_t(4); 2*q_t(2)*q_t(3)-2*q_t(1)*q_t(4), 1-2*q_t(2)^2 - 2*q_t(4)^2];
    At = Rot_t*((1-t)*eye(2) +  t*S{4,i});
    b(4*(i-1)+1:4*(i-1)+4)=[At(1,1), At(1,2), At(2,1), At(2,2)]'; %build up matrix b, for min ||Ax-b||.
...
b(4*length(FV1)+1)= (1-t)*fixed_vertices(4,1)+t*fixed_vertices(4+1,1);  %constraints for fixed vertex.
b(4*length(FV1)+2)= (1-t)*fixed_vertices(4,2)+t*fixed_vertices(4+1,2);
A(4*length(FV1)+1,1) = 1;
A(4*length(FV1)+2,2)=1;
...
for i=1:length(FV1) %calculate A for each triangle.
    ...
    P5 = [V_5(FV1(i,1),1), V_5(FV1(i,1),2); V_5(FV1(i,2),1), V_5(FV1(i,2),2); V_5(FV1(i,3),1), V_5(FV1(i,3),2)];
    P6 = [V6(FV1(i,1),1), V6(FV1(i,1),2); V6(FV1(i,2),1), V6(FV1(i,2),2); V6(FV1(i,3),1), V6(FV1(i,3),2)];
